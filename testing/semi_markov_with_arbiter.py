...
class Mouse(object):
    ...
    def compute_drive(self, current_state, actor_policy):
        for action_num in range(self.env.n_actions):
            other_actions = np.where(np.arange(self.env.n_actions) != action_num)[0]
            self.actor_value - actor_policy[other_actions]
    ...
    def one_trial(self):
        ...
        while not self.env.in_terminal_state() and t < 1000:
            ...
            actor_policy = self.softmax(self.actor_value[:, current_state_num])
            drive = self.compute_drive(current_state, actor_policy)
            a = self.choose_action(actor_policy, dwell_time)
...
if __name__ == '__main__':
    ...
    e = Box()
    a = Mouse(env=e, critic_learning_rate=0.01, actor_learning_rate=0.01, habitisation_rate=0.001)
