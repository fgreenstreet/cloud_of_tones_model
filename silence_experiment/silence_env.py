...
class SilenceBox(object):
    """
    Environment for simulating the silence experiment. Note that we use the same setup as for the white noise experiment
    """
    def __init__(self, punish=False):
        ...
        self.env_states = ['Start', 'High', 'Low', 'Outcome', 'Silence']
        self.actions = ['Left', 'Centre', 'Right', 'Idle']
        action_states = ['HighLeft', 'HighRight', 'LowLeft', 'LowRight', 'SilenceLeft', 'SilenceRight']
    ...
    def get_reward(self, state, action, next_state):
        if next_state != 'Outcome':
            ...
            if self.high_to_left:
                if state == 'HighLeft' or state == 'LowRight' or state == 'SilenceLeft':
                    reward_amount = 1
                    trial_type = 'normal'
                else:
                    reward_amount = 0
                ...
                if state == 'HighRight' or state == 'LowLeft' or state == 'SilenceRight':
                    reward_amount = 1
                    trial_type = 'normal'
                else:
                    reward_amount = 0
    ...
    def get_next_state(self, state, action, timer_is_not_up, trial_num):
        ...
        elif state == 'High' or state == 'Low' or state == 'Silence':
            if action == 'Idle':
                next_state = state
            elif action == 'Centre':
                if self.punish:
        ...
        elif state == 'HighLeft' or state == 'HighRight' or state == 'LowLeft' or state == 'LowRight' or state == 'SilenceLeft' or state == 'SilenceRight':
            if timer_is_not_up:
                next_state = state
            else:
                next_state = 'Outcome'
